...
class PrompDelayModel( object ):
    ...
    # -------- #
    # STRETCH
    def set_distprop_stretch(self, mu1=0.37, sigma1=0.61, mu2=-1.22, sigma2=0.56, a=0.51):
    ...
    def set_distprop_color(self, mu=-0.030, sigmaup=0.086, sigmadown=0.052,
                               mu_delayed=None, sigmaup_delayed=None, sigmadown_delayed=None):
        """ Set the parameters of the SNe Ia color distribution, modeled as asymetric gaussians. 
        
        If *_delayed are not provided, prompt and delayed are assumed similar (tested on mu_delayed)
        """
        ...
        self._distprop_color = {"prompt":{"mu":mu, "sigmaup":sigmaup, "sigmadown":sigmadown},
                                "delayed":{"mu":mu_delayed, "sigmaup":sigmaup_delayed, "sigmadown":sigmadown_delayed}
                               }
    
    # HOST MASS
    def set_distprop_mass(self, mu=9.61, sigmaup= 0.67, sigmadown=0.71,
                                 mu_delayed=10.79, sigmaup_delayed= 0.52, sigmadown_delayed=0.82):
        """ Set the parameters of the SNe Ia color distribution, modeled as asymetric gaussians. 
        """
        self._distprop_mass = {"prompt":{"mu":mu, "sigmaup":sigmaup, "sigmadown":sigmadown},
                                "delayed":{"mu":mu_delayed, "sigmaup":sigmaup_delayed, "sigmadown":sigmadown_delayed}
                               }
    ...
    def set_distprop_hr(self, mean_prompt=0.075, sigma_prompt= 0.1,
        ...
        self._distprop_hr = {"prompt":{"mean":mean_prompt, "sigma":sigma_prompt},
                                "delayed":{"mean":mean_delayed, "sigma":sigma_delayed}
                               }
    ...
    def get_distpdf_stretch(self, x, fprompt, dx=None, **kwargs):
        ...
        **kwargs goes to set_distprop_stretch()
        ...
        mode1_pdf = stats.norm.pdf(x, loc  = self.distprop_stretch["mode1"]["loc"],
                                      scale= np.sqrt(self.distprop_stretch["mode1"]["scale"]**2 + dx**2)
                                  )
        mode2_pdf = stats.norm.pdf(x, loc  = self.distprop_stretch["mode2"]["loc"],
                                      scale= np.sqrt(self.distprop_stretch["mode2"]["scale"]**2 + dx**2)
                                  )
        
        return fprompt * mode1_pdf + (1-fprompt) * (self.distprop_stretch["a"]*mode1_pdf + (1-self.distprop_stretch["a"])*mode2_pdf)
    ...
    def get_distpdf_color(self, x, fprompt, dx=None, **kwargs):
        ...
        **kwargs goes to set_distprop_color()
        ...
        prompt = asym_gaussian(x, *list(self.distprop_color["prompt"].values()), dx=dx)
        delayed = asym_gaussian(x, *list(self.distprop_color["delayed"].values()), dx=dx)
    ...
    def get_distpdf_mass(self, x, fprompt, dx=None, z=None, **kwargs):
        ...
        **kwargs goes to set_distprop_mass()
        ...
        if z is not None:
            raise NotImplementedError("No redshift dependency implemented for get_distpdf_mass(). Set z=None")

        prompt = asym_gaussian(x, *list(self.distprop_mass["prompt"].values()), dx=dx)
        delayed = asym_gaussian(x, *list(self.distprop_mass["delayed"].values()), dx=dx)
        return fprompt*prompt + (1-fprompt) * delayed
